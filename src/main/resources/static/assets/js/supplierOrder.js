	   ...
	   $.ajax({
	   	...
	   	success: function(res){
	   		if(res.request == "SUCCESS"){
				...
				$("#modal_footer").empty();
				if(supplierOrder.state == "已确认"){
				   var closeBtn = $("<a href='#'  class='btn btn-secondary'  data-dismiss='modal'>关闭</a>");
				   $("#modal_footer").append(closeBtn);
				}else{
				   var okBtn = $("<a href='javascript:void(0)' onclick='okOrder("+supplierOrderId+")' class='btn btn-secondary'>确定订单</a>");
				   var closeBtn = $("<a href='javascript:void(0)' onclick='openErrorModel("+supplierOrderId+")' class='btn btn-danger'>订单有误</a>");
